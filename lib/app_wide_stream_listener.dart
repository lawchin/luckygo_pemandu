  ...
  Future<void> _initAndListen() async {
    ...
    _ratingsSub = query.snapshots().listen(
      (qs) async {
        ...
        // ⏳ Delay logic: only show rating if it's older than 10 minutes
        if (ts != null) {
          final now = DateTime.now();
          final ratingTime = ts.toDate();
          final diff = now.difference(ratingTime);
...
// CHANGE TIMER HERE ######################################################################################################
          if (diff.inMinutes < 10) {
            _log('Rating is too fresh (${diff.inMinutes} min). Will wait until 10 minutes have passed.');
            return;
          }
        }
// CHANGE TIMER HERE ######################################################################################################
