...
class Bucket123b extends StatefulWidget {
  const Bucket123b({super.key, required this.bucketIndex});

  /// This page is used for 1..3 (ROAD buckets). If used with >3 it will still work (AIR fallback).
  final int bucketIndex;

  @override
  State<Bucket123b> createState() => _Bucket123bState();
}

class _Bucket123bState extends State<Bucket123b> {
  ...
  // prevent double-push of View15
  bool _openingView15 = false;
  ...
  Future<bool> _onBlockBack() async {
    // Block all attempts to pop this page via device back/system gestures.
    // Optional UX: tell the user this page cannot be dismissed via back.
    if (mounted) {
...
appBar: AppBar(
  title: Text(_title),
  automaticallyImplyLeading: true, // show the default back chevron
  leading: IconButton(
    icon: const Icon(Icons.arrow_back),
    onPressed: () {
      // you can decide whether to allow Navigator.pop here
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        Gv.tips1Amount       = (p.length > 33) ? _toInt(p, 33) : 0;
                        Gv.tips2Amount       = (p.length > 34) ? _toInt(p, 34) : 0;
                        ...
                        return Card(
                          ...
                          child: InkWell(
                            ...
                            onTap: () async {
                              ...
                              // --- Addresses (7..10) ---
                              Gv.sAdd1 = sAdd1;  Gv.sAdd2 = sAdd2;
                              Gv.dAdd1 = dAdd1;  Gv.dAdd2 = dAdd2;

                              // --- Coordinates (11..14) ---
                              Gv.sLat = _toDbl(p, 11);  Gv.sLng = _toDbl(p, 12);
                              Gv.dLat = _toDbl(p, 13);  Gv.dLng = _toDbl(p, 14);
                              ...
                              try {
                                ...
                                final won = await FirebaseFirestore.instance.runTransaction<bool>((tx) async {
                                  final snap = await tx.get(jobDocRef);
                                  final data = (snap.data() ?? {}) as Map<String, dynamic>;
                                  final v = data[Gv.liteJobId];
                                  if (v is! String) return false;

                                  // capture string for View15 + restore if needed
                                  Gv.liteJobData = v;

                                  // atomically remove from pool so others can't take it
                                  tx.update(jobDocRef, {Gv.liteJobId: FieldValue.delete()});
                                  return true;
                                }).catchError((_) => false);
                                ...
                                if (!won) {
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(content: Text('This job was just taken by another driver.')),
                                    );
                                  }
                                  return;
                                }
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Gv.tips1Amount == 0
                                          ...
                                          : (Gv.tips1Amount > 0 && Gv.tips2Amount == 0
                                              ? Container(
                                                  height: 26,
                                                  decoration: BoxDecoration(
                                    ...
                                    children: [
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          ...
                                          const Text('⟶',
                                              style: TextStyle(height: 0.1, color: Colors.red, fontSize: 30)),
                                        ],
                                      ),
                                      const SizedBox(width: 6),
  ...
  Future<void> _ensureRoadForParsed(
    ...
    for (final e in parsed) {
      ...
      if (Gv.roadByJob.containsKey(key)) continue;
      if (_inFlightKeys.contains(key)) continue;
...
// //               // CHANGE: accept 33 or more tokens (so 35 works)
// //               if (parts.length < 33) { badRows++; return; }
...
// //                             if (data != null && data.containsKey(Gv.liteJobId)) {
// //                               // Save the job data into your global variable
// //                               Gv.liteJobData = data[Gv.liteJobId];

// //                               // Now proceed with deletion
// //                               await jobDocRef.update({Gv.liteJobId: FieldValue.delete()});
// //                             }
