...
import 'package:keep_screen_on/keep_screen_on.dart'; // keep screen awake
...
class _FilterJobsOneStreamState extends State<FilterJobsOneStream>
  ...
  // ── Keep screen on ──────────────────────────────────────────────────────
  Future<void> _ensureScreenOn() async {
    try {
      await KeepScreenOn.turnOn();
      // await KeepScreenOn.turnOn(withAllowLockWhileScreenOn: true); // optional Android flag
    } catch (e) {
      debugPrint('[KEEP_SCREEN_ON] turnOn error: $e');
    }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      _ensureScreenOn(); // re-apply after background on iOS
    }
  }
  ...
  void initState() {
    ...
    // Observe app lifecycle + keep screen on
  ...
  @override
  void dispose() {
    try { KeepScreenOn.turnOff(); } catch (_) {}
  ...
  Future<void> _maybeAutoNavigate({bool force = false}) async {
    ...
    try {
      ...
      // onJob = d['driver_is_on_a_job'] == true; // optional
    ...
    // ✅ Null-safety promotion: use a non-null local `job`
    final job   = best!;
    ...
    try {
      ...
      final ok = await FirebaseFirestore.instance.runTransaction<bool>((tx) async {
        ...
        tx.update(driverRef, {'job_auto': false});
