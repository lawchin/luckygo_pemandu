...
onTap: (r.count ?? 0) <= 0
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(r.range!, maxLines: 1, overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 10,
  ...
  Future<void> _fetchRoadForCurrentShortlist() async {
    ...
    debugPrint('[FJOS] ROAD fetch: shortlist=${_shortlist.length}, missing=${missing.length}, cache=${_roadKmCache.length}, inFlight=${_inFlight.length}');
  ...
  void _rebuildRoadOverlayFromCache() {
    ...
    debugPrint('[FJOS] overlay123 from cache: $overlay  (cache=${_roadKmCache.length})');
  ...
  Future<Map<String, double>> _callDistanceMatrix({
    ...
    if (apiKey.isEmpty || jobs.isEmpty) {
      debugPrint('[FJOS] DistanceMatrix: apiKey empty? ${apiKey.isEmpty}, jobs=${jobs.length}');
      return {};
    }
    ...
    try {
      debugPrint('[FJOS] DM call → ${uri.toString().substring(0, 120)}...  (dest=${jobs.length})');
      ...
      if (elements.isNotEmpty) {
        final firstStatus = (elements.first as Map<String, dynamic>?)?['status'];
        debugPrint('[FJOS] DM elem[0] status: $firstStatus  (n=${elements.length})');
      }
      ...
      for (var i = 0; i < n; i++) {
        ...
        out[key] = double.parse(km.toStringAsFixed(1));
      }
  ...
  void _startLocationPolling() {
    ...
    _locPollTimer = Timer.periodic(_pollInterval, (_) {
      ...
      final movedM = _haversineKm(_anchorLat, _anchorLng, curLat, curLng) * 1000.0;
  ...
  void _scheduleReanchor(double lat, double lng) {
    ...
    _reanchorDebounce = Timer(_stopDebounce, () {
      ...
      final distM = _haversineKm(_anchorLat, _anchorLng, newLat, newLng) * 1000.0;
      if (distM < _reanchorMeters) return; // in case we drifted back

      debugPrint('[FJOS] Re-anchoring: old=(${_anchorLat.toStringAsFixed(6)},${_anchorLng.toStringAsFixed(6)}) '
  ...
  void _dumpBucket4Jobs({required String reason}) {
    ...
    debugPrint('[FJOS] DUMP bucket4Jobs ($reason): count=${list.length} builtAt=${Gv.bucket4LastBuiltAt} ver=${Gv.bucket4Version.value}');
    for (var i = 0; i < list.length && i < 50; i++) {
      final j = list[i];
      final key = _cacheKey(j.jobId, j.sLat, j.sLng, _anchorLat, _anchorLng);
      debugPrint('  [$i] jobId=${j.jobId} s=(${j.sLat.toStringAsFixed(6)},${j.sLng.toStringAsFixed(6)}) key=$key');
    }
    if (list.length > 50) {
...
class _RowSpec {
  ...
  _RowSpec._bucket(this.index, this.name, this.range, this.count, this.pill, this.pillColor);
